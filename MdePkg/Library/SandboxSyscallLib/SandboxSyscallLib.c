...
mCreateEvent(IN UINT32 Type, IN EFI_TPL NotifyTpl,
             ...
             OUT EFI_EVENT *Event) {
  return SandboxSyscall1(SANDBOX_SYS_NULL, SANDBOX_SYS_BS_CREATE_EVENT);
}

EFI_STATUS
EFIAPI
mSetTimer(IN EFI_EVENT Event, IN EFI_TIMER_DELAY Type, IN UINTN TriggerTime) {
  return SandboxSyscall1(SANDBOX_SYS_NULL, SANDBOX_SYS_BS_SET_TIMER);
}

EFI_STATUS
EFIAPI
mWaitForEvent(IN UINTN NumberOfEvents, IN EFI_EVENT *Event, OUT UINTN *Index) {
  return SandboxSyscall1(SANDBOX_SYS_NULL, SANDBOX_SYS_BS_WAIT_FOR_EVENT);
}

EFI_STATUS
EFIAPI
mSignalEvent(IN EFI_EVENT Event) {
  return SandboxSyscall1(SANDBOX_SYS_NULL, SANDBOX_SYS_BS_SIGNAL_EVENT);
}

EFI_STATUS
EFIAPI
mCloseEvent(IN EFI_EVENT Event) {
  return SandboxSyscall1(SANDBOX_SYS_NULL, SANDBOX_SYS_BS_CLOSE_EVENT);
}

EFI_STATUS
EFIAPI
mCheckEvent(IN EFI_EVENT Event) {
  return SandboxSyscall1(SANDBOX_SYS_NULL, SANDBOX_SYS_BS_CHECK_EVENT);
}
...
EFI_STATUS
EFIAPI
SandboxSyscallLibConstructor(IN EFI_HANDLE ImageHandle,
