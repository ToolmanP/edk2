...
/**
  Convert VMR_PROP_T flags (VMR_READ/VMR_WRITE/VMR_EXEC/VMR_DEVICE/VMR_NOCACHE/VMR_COW) 
  to IA32_MAP_ATTRIBUTE suitable for EDK2 page-table usage.

  @param[in]  VmrProp  The combination of VMR_* flags.
  @param[in]  KernelVMR  Whether the VMR is for kernel.

  @return     IA32_MAP_ATTRIBUTE representing the corresponding page table attributes.
**/
IA32_MAP_ATTRIBUTE
ConvertVmrToMapAttr (
  IN UINT32  VmrProp,
...
VOID PrintPageTable(UINT64 PageTable)
{
  ...
  for (UINTN Index = 0; Index < MapCount; Index++) {
    SBDebug("Map[%d]: LinearAddress: 0x%lx, Length: 0x%lx, Attribute: 0x%lx\n",
            Index, Map[Index].LinearAddress, Map[Index].Length, Map[Index].Attribute.Uint64);
  }
...
EFI_STATUS InitCorePageTable(UINT64 *CorePageTablePtr)
{
  ...
  Status = MapRangeInPageTable(
      CorePageTablePtr, PhysicalStart,
      PHYS_TO_VIRT(PhysicalStart),
      PHYS_TO_VIRT(PhysicalEnd),
